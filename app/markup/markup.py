...
from app.markup.buttons import Btn

def create_start_markup(user_id: int) -> InlineKeyboardMarkup:
    count_products = get_count_all_products()
    count = ceil(count_products / 5)
    markup = InlineKeyboardMarkup().add(InlineKeyboardButton(text=Btn.web_site, callback_data='web_site'))
    markup.add(InlineKeyboardButton(text=Btn.video, callback_data='description'))
    markup.add(InlineKeyboardButton(text=Btn.catalog, callback_data="{\"page\":\"catalog\",\"act\":\"pagin\",\"PageNum\":\"1\",\"CountPage\":"+ str(count) +"}"))
    if check_user_baket_exist(user_id):
        user_products_count = ceil(len(get_cart_by_user(user_id)) / 5)
        markup.add(InlineKeyboardButton(text=Btn.cart, callback_data="{\"page\":\"cart\",\"act\":\"pagin\",\"PageNum\":\"1\",\"CountPage\":" + str(user_products_count) + "}"))
...
def create_catalog_markup(query: CallbackQuery) -> InlineKeyboardMarkup:
    ...
    if "pagin" in request[0]:
        ...
        markup.add(InlineKeyboardButton(text=Btn.menu, callback_data='menu'))
...
def create_cart_markup(query: CallbackQuery) -> InlineKeyboardMarkup:
    ...
    if 'pagin' in request[0]:
        ...
        markup.add(InlineKeyboardButton(text=Btn.menu, callback_data='menu'),
                InlineKeyboardButton(text=Btn.delete_backet, callback_data='delete_backet'),
                InlineKeyboardButton(text=Btn.pay_card, callback_data='payment'))
...
def create_selected_item_markup(query: CallbackQuery) -> InlineKeyboardMarkup:
    ...
    markup = InlineKeyboardMarkup().add(InlineKeyboardButton('Убрать из корзины', callback_data="{\"act\":\"reduce\",\"userId\":" + str(query.message.chat.id)+ ",\"orderId\":" + str(selected_item[0].id) +"}"),
        ...
        .add(InlineKeyboardButton(text=Btn.cart, callback_data="{\"page\":\"cart\",\"act\":\"pagin\",\"PageNum\":" + str(data.page - 1)+ ",\"CountPage\":" + str(data.count_page)+"}"))
    return markup
...
def create_product_card_markup(query: CallbackQuery) -> InlineKeyboardMarkup:
    ...
    if user_products_count:
        markup.add(InlineKeyboardButton(text=Btn.cart, callback_data="{\"page\":\"cart\",\"act\":\"pagin\",\"PageNum\":\"1\",\"CountPage\":" + str(user_products_count) + "}"))
    markup.add(InlineKeyboardButton(text=Btn.catalog, callback_data="{\"page\":\"catalog\",\"act\":\"pagin\",\"PageNum\":" + str(data.page - 1)+ ",\"CountPage\":" + str(data.count_page)+"}"))
    return markup


def create_invoice_markup(query: CallbackQuery) -> InlineKeyboardMarkup:
    user_products_count = ceil(len(get_cart_by_user(query.message.chat.id)) / 5)
    markup = InlineKeyboardMarkup().add(InlineKeyboardButton(text=Btn.pay_card,pay=True))\
        .add(InlineKeyboardButton(text=Btn.cart, callback_data="{\"page\":\"cart\",\"act\":\"pagin\",\"PageNum\":\"1\",\"CountPage\":" + str(user_products_count) + "}"))\
        .add(InlineKeyboardButton(text=Btn.menu, callback_data='menu'))
    return markup
